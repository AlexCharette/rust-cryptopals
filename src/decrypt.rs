...
pub fn dec_aes128_ecb_to_bytes(
    bytes: &[u8],
    key: &[u8],
    pad: bool,
) -> Result<DecryptResult<Vec<u8>>, SymmetricCipherError> {
    info!("Input {:?}\nInput Length: {:?}", bytes, bytes.len());

    let mut decryptor = if pad {
        ecb_decryptor(KeySize::KeySize128, key, blockmodes::PkcsPadding)
    } else {
        ecb_decryptor(KeySize::KeySize128, key, blockmodes::NoPadding)
    };
...
pub fn dec_aes128_ecb_to_string(
    bytes: &[u8],
    key: &[u8],
    pad: bool,
) -> Result<DecryptResult<String>, SymmetricCipherError> {
    info!("Input {:?}\nInput Length: {:?}", bytes, bytes.len());
...
pub fn dec_aes128_ecb_b64(
    b64_str: &str,
    key: &str,
    iv: &[u8],
) -> Result<DecryptResult<String>, DecodeError> {
    ...
    let mut decryptor = ecb_decryptor(KeySize::KeySize128, key.as_bytes(), blockmodes::PkcsPadding);
    let mut input = RefReadBuffer::new(&bytes);
    let mut out_buff = &mut [0u8; 16];
    let mut output = RefWriteBuffer::new(out_buff);
    decryptor
        .decrypt(&mut input, &mut output, false)
...
pub fn dec_repeat_xor(b64_str: &str) -> Result<DecryptResult<String>, DecodeError> {
    ...
    for keysize in 2..=40 {
        let hamming_dists: Vec<i32> = bytes
            .chunks(keysize * 2)
            .map(|chunk| {
                super::get_hamming_dist(&chunk[0..chunk.len() / 2], &chunk[chunk.len() / 2..])
            })
            .collect();
